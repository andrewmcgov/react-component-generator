...
export function activate(context: ExtensionContext) {
	...
	const generateComponent = commands.registerCommand('react-component-generator.generateComponent', async () => {
		// Code in here is executed every time the command is executed
		const componentName = await window.showInputBox();

		if (componentName) {
			window.showInformationMessage(`Your component will be called ${componentName}.`);
